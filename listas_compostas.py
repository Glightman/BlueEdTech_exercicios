...
contagem = 0
só_peso = list()
while Quer_continuar == 0:
   dado = list()
   dado.append(str(input('Seu nome: ')))
   dado.append(float(input('Peso: ')))
   pessoas.append(dado[:])
   contagem += 1
   Quer_continuar = int(input('''Deseja continuar?
   [ 0 ] SIM
   [ 1 ] NÃO
   -->: '''))
for m in pessoas:
   só_peso.append(m[1])
print('Foram cadastradas {} pessoas'.format(contagem))
print('O maior peso foi {} Kg'.format(max(só_peso)))
print('O menor peso foi {} Kg'.format(min(só_peso)))
